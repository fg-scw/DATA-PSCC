...
resource "scaleway_object_bucket" "livrables" {
  ...
  # Object Lock uniquement si retention > 0
  object_lock_enabled = var.data_retention_days > 0

  tags = {
    environment = "hackathon"
    purpose     = "livrables"
    #retention   = "${var.data_retention_days}days"
    retention   = var.data_retention_days > 0 ? "${var.data_retention_days}days" : "none"
