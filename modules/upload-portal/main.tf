#===============================================================================
# UPLOAD PORTAL MODULE - VM-based (FIXED - htpasswd encoding + user_roles)
...
locals {
  ...
  # Combine provider, evaluator and admin credentials for htpasswd
  htpasswd_entries = concat(
    ...
    [
      ...
      "${local.admin_credentials.username}:${bcrypt(local.admin_credentials.password)}"
    ]
  )
  ...
  # User roles content - built separately to avoid YAML issues
  user_roles_lines = concat(
    ["evaluator:evaluator", "admin:admin"],
    [for key, creds in local.provider_credentials : "${creds.username}:provider"]
  )
  user_roles_content = join("\n", local.user_roles_lines)
  user_roles_base64  = base64encode(local.user_roles_content)
}
...
write_files:
  ...
  - path: /opt/upload-portal/user_roles
    permissions: '0644'
    encoding: b64
    content: ${local.user_roles_base64}
    ...
    content: |
      ...
      async function loadFiles() {
        ...
        try {
          ...
          if (d.success && d.files.length > 0) {
            ...
            d.files.forEach(f => { html += '<tr><td><span class="file-icon">📄</span>' + f.key + '</td><td>' + f.size + '</td><td>' + f.modified + '</td></tr>'; });
            html += '</tbody></table>';
        ...
        try {
          ...
          if (d.success && d.files.length > 0) {
            ...
            d.files.forEach(f => {
              ...
              html += '<td><span class="file-icon">📦</span>' + f.filename + '</td>';
              html += '<td>' + f.size + '</td>';
      ...
      def download_livrable():
          ...
          try:
              ...
              # Try without SSE-C first (for unencrypted files)
              try:
                  response = s3.get_object(Bucket=bucket, Key=key)
              except ClientError as e:
                  error_code = e.response.get('Error', {}).get('Code', '')
                  # If InvalidRequest, the file is encrypted - retry with SSE-C
                  if error_code == 'InvalidRequest':
                      sse = sse_params('livrables')
                      if sse:
...
# Access to browse uploaded files and download livrables
...
#-------------------------------------------------------------------------------
# Store admin credentials for portal
#-------------------------------------------------------------------------------
resource "local_sensitive_file" "admin_portal_credentials" {
  filename        = "${path.root}/keys/admin/portal_credentials.txt"
  file_permission = "0600"
  content         = <<-EOT
# Upload Portal Credentials - Admin
...
## Debug endpoints
Health check: curl http://${scaleway_instance_ip.portal.address}/health
Debug S3: curl -u ${local.admin_credentials.username}:${local.admin_credentials.password} http://${scaleway_instance_ip.portal.address}/debug
EOT
}
